	...
	if(isset($_POST['save']))
	  {
		...
		$result = $db->exec("INSERT INTO bank_account014(ID,bank,branch,ac_type,name_ac,sex,status,address) VALUES('$n','$b','$br','$at','$ac','$s','$st','$ad')");
      ...
      if(isset($_GET['del'])){
		$id=$_GET['del'];
		$stmt = $db->prepare("DELETE FROM bank_account014 WHERE ID=:id");
	 ...
	 if(isset($_POST['edit']))
	 {
	   ...
	   $affected_rows = $db->exec("UPDATE bank_account014 SET **name='$l',Reg='$r',Class='$c',Section='$s' where ID='$n'");
	   $insertId = $db->lastInsertId();
...
<div class="editbox">
    <?php  if(isset($_GET['edit'])){
		$id=$_GET['edit'];
		foreach($db->query("select * from thai where ID ='$id'")as $r){
    ?>
...
?>
    <form method="post" action="">
        <div class="form-row">
            ...
            <div id="input" class="col">
                <label for="cars">เลือกธนาคาร</label>
                <select name="cars" id="bank">
                    <option value="volvo">ธนาคารกรุงไทย</option>
                    <option value="saab">ธนาคารกรุงเทพ</option>
                    <option value="opel">ธนาคารไทยพาณิชย์</option>
                    <option value="audi">ธนาคารกสิกรไทย</option>
                </select>
            </div>
            <div id="input" class="col">
    ...
    <div class="table-responsive-xxl">
        <table class="table table-striped">
            ...
            <?php 
		foreach($db->query("select * from thai") as $row) {
			$num = $row["ID"];
	?>
            <tr>
