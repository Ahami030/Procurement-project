	...
	if(isset($_POST['save']))
	  {
		...
		$result = $db->exec("INSERT INTO eng(ID,NAME,Reg,Class,Section) VALUES('$n','$l','$r','$c','$s')");
      ...
      if(isset($_GET['del'])){
		$id=$_GET['del'];
		$stmt = $db->prepare("DELETE FROM eng WHERE ID=:id");
	 ...
	 if(isset($_POST['edit']))
	 {
        ...
        $c = $_POST['Class'];
        $s = $_POST['Section'];
	   $affected_rows = $db->exec("UPDATE eng SET  name='$l',Reg='$r',Class='$c',Section='$s' where ID='$n'");
	   $insertId = $db->lastInsertId();
...
<div class="editbox">
    <?php  if(isset($_GET['edit'])){
		$id=$_GET['edit'];
		foreach($db->query("select * from eng where ID ='$id'")as $r){
    ?>
...
?>
    ...
    <div class="table-responsive-xxl">
        <table class="table table-striped">
            ...
            <?php 
		foreach($db->query("select * from eng") as $row) {
			$num = $row["ID"];
	?>
            <tr>
