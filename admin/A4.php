	...
	if(isset($_POST['save']))
	  {
		...
		$result = $db->exec("INSERT INTO program(ID,NAME,Reg,Class,Section) VALUES('$n','$l','$r','$c','$s')");
      ...
      if(isset($_GET['del'])){
		$id=$_GET['del'];
		$stmt = $db->prepare("DELETE FROM program WHERE ID=:id");
	 ...
	 if(isset($_POST['edit']))
	 {
        ...
        $c = $_POST['Class'];
        $s = $_POST['Section'];
	   $affected_rows = $db->exec("UPDATE program SET  name='$l',Reg='$r',Class='$c',Section='$s' where ID='$n'");
	   $insertId = $db->lastInsertId();
...
<div class="editbox">
    <?php  if(isset($_GET['edit'])){
		$id=$_GET['edit'];
		foreach($db->query("select * from program where ID ='$id'")as $r){
    ?>
