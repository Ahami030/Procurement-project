...
// Function to display the table in the preview div with editable text inputs
function displayTablePreview(tableHTML) {
    document.getElementById('preview').innerHTML = tableHTML;

    // Replace each table cell with an editable text input
    var cells = document.querySelectorAll('#preview td');
    cells.forEach(function(cell) {
        var textInput = document.createElement('input');
        textInput.type = 'text';
        textInput.value = cell.textContent;
        cell.textContent = '';
        cell.appendChild(textInput);

        // Update the table cell content when the text input value changes
